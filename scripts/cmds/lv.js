 ...
 * Riyad Bot Framework
 *
 * Search up to 10 lyrics videos via the user's own riyad-video-api, send ONE
 * numbered collage image, then download the selected lyrics video as mp4.
 *
...
module.exports = {
  config: {
    ...
    category: "music",
    description: {
      en: "Search a lyrics video and download it by replying with a number",
    },
  ...
  onStart: async function ({ api, event, args }) {
    ...
    try {
      ...
      if (!results.length) {
        react(api, "❌", messageID);
        return api.sendMessage(
          `"${query}" এর জন্য কোনো lyrics video পাওয়া যায়নি।`,
          threadID,
          messageID,
        );
      }
      ...
      react(api, "❌", messageID);
      console.error("[LV ERROR]", error.message);
  ...
  onReply: async function ({ api, event, Reply }) {
    ...
    try {
      ...
      if (fs.existsSync(filePath)) fs.unlinkSync(filePath);
      react(api, "❌", messageID);
      console.error("[LV DOWNLOAD ERROR]", error.message);
      return api.sendMessage(
