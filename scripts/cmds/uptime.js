...
module.exports = {
  ...
  onStart: async function({ api, event, message, usersData, threadsData }) {
    ...
    let response = `📊 ━━━━ [ ${config.botName.toUpperCase()} SYSTEM ] ━━━━ 📊\n`;
    response += `⏱️ Uptime: ${uptimeStr}\n`;
    response += `📁 Commands: ${commandLoader.commands.size} active | ${commandLoader.aliases.size} aliases\n`;
    response += `👥 Active Database: ${totalUsers} users | ${totalThreads} groups\n`;
    response += `📟 Memory usage: ${memoryUsage}\n`;
    response += `⚙️ Node.js version: ${process.version}\n`;
    response += `💻 CPU Core info: ${cpuCores}x ${cpuModel}\n`;
